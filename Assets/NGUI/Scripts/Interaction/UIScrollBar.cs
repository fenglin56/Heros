...
public class UIScrollBar : MonoBehaviour
{
    ...
    //Add By Jiang
    public Vector2 ForegroundScaleSize;
    public float ForegroundZDepth;
    public UIFilledSprite ProgressBackground;
    ...
    void SetScorell(float value)
    {
        ...
        {
            float nearlyValue = 1;
            StepList.ApplyAllItem(P=>nearlyValue = Mathf.Abs(P-value)<Mathf.Abs(nearlyValue-value)?P:nearlyValue);
            value = nearlyValue;
        }
        mScroll = value;
        mIsDirty = true;
        if (onChange != null) onChange(this);
        if (ProgressBackground != null) { ProgressBackground.fillAmount = value; }
    }
	...
	void Start ()
	{
        ...
        {
            ProgressBackground.fillAmount = scrollValue;
            ProgressBackground.fillDirection = UISprite.FillDirection.Horizontal;
        }
	}
	...
	public void ForceUpdate ()
	{
		...
		if (mBG != null && mFG != null)
		{
			...
			if (mDir == Direction.Horizontal)
			{
                Vector2 fgs = new Vector2(bgs.x * mSize* ForegroundScaleSize.x, bgs.y * ForegroundScaleSize.y);

				mFG.pivot = UIWidget.Pivot.Left;
				mBG.pivot = UIWidget.Pivot.Left;
				mBG.cachedTransform.localPosition = Vector3.zero;
                mFG.cachedTransform.localPosition = new Vector3(bg.x - fg.x + (bgs.x - fgs.x) * val, 0f, ForegroundZDepth);
			...
			{
                Vector2 fgs = new Vector2(bgs.x * ForegroundScaleSize.x, bgs.y * mSize * ForegroundScaleSize.y);

				mFG.pivot = UIWidget.Pivot.Top;
				mBG.pivot = UIWidget.Pivot.Top;
				mBG.cachedTransform.localPosition = Vector3.zero;
                mFG.cachedTransform.localPosition = new Vector3(0f, -bg.y + fg.y - (bgs.y - fgs.y) * val, ForegroundZDepth);
				mFG.cachedTransform.localScale = new Vector3(fgs.x + fg.x + fg.z, fgs.y + fg.y + fg.w, 1f);
				if (val < 0.999f && val > 0.001f) mFG.MakePixelPerfect();
			}
		}
