...
public class UIInput : MonoBehaviour
{
	...
	void UpdateLabel ()
	{
        ...
        string[] mSingleText = new string[mText.Length];
        int myCByteLenght = 0;
        int myEByteLenght = 0;
        int getByteLenght;
        Regex rx = new Regex("^[a-z0-9]$");
        //Regex rxSymbol = new Regex("^([\u4e00-\u9fa5]+|[a-zA-Z0-9]+)$");
        for (int i = 0; i < mSingleText.Length; i++)
        {
            mSingleText[i] = mText.Substring(i, 1);
            ...
            getByteLenght = myCByteLenght * 2 + myEByteLenght;
            if (getByteLenght > maxChars)
            {
